...
-- 1. Write a query to find the top 5 most frequently ordered dishes by customer called "Tushar Sampath"
-- in the last 1 year.

Create View top_5_frequently_ordered_dishes_for_specific_customer AS
SELECT *
FROM 
(
	SELECT 
	...
	WHERE
		...
		AND
		customers.customer_name = 'Tushar Sampath'
	GROUP BY 
		customers.customer_id,
		customers.customer_name,
		orders.order_item
...
Create View popular_time_slots_2hr_interval AS
SELECT
	CASE
...
Create View order_value_analysis AS
SELECT 
	customers.customer_name,
...
Create View orders_without_delivery AS
SELECT 
...
Create View restaurant_revenue_ranking AS
WITH ranking_table
AS 
	(SELECT 
...
Create View most_popular_dish_by_city AS
SELECT *
FROM 
	(SELECT 
		restaurants.city,
...
Create View cancellation_rate_comparasion AS
...
Create View rider_avg_delivery_time AS
SELECT 
	d.rider_id,
    AVG(
		TIMESTAMPDIFF(
...
Create View monthly_restaurant_growth_ratio_1 AS
...
Create View rider_monthly_earnings AS
SELECT
	deliveries.rider_id,
...
Create View rider_rating_analysis AS
...
Create View order_frequency_by_day AS
SELECT
	restaurant_id,
    restaurant_name,
    day AS peak_day,
...
Create View rider_efficiency AS
SELECT 
...
Create View order_item_popularity AS
SELECT
	order_item,
    CASE
...
Create View monthly_restaurant_growth_ratio_2 AS
